...
import org.meghana.OnboardingAndAssetsMgmt.payload.auth.*;
...
import org.meghana.OnboardingAndAssetsMgmt.service.TokenService;
import org.meghana.OnboardingAndAssetsMgmt.util.constants.AccountError;
...
public class AuthController {
    ...
    @Autowired
    private RedisInvalidatedTokenService redisInvalidatedTokenService;
...
//            log.info("here");
            ...
            if (optionalAccount.isPresent()) {
                ...
                if (account.isApproved()) {
                    ...
                    return new ResponseEntity<>(new TokenDTO("Un-Authorized. Your account is pending approval."), HttpStatus.UNAUTHORIZED);
                }
            } else {
                return new ResponseEntity<>(new TokenDTO(" Un-Authorized. Your account does not exist."), HttpStatus.BAD_REQUEST);
            }
        } catch (AuthenticationException e) {
            log.debug(AccountError.TOKEN_GENERATION_ERROR.toString() + ": " + e.getMessage());
            return new ResponseEntity<>(new TokenDTO("Un-Authorized. No Token returned."), HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<String> logout(@RequestBody TokenDTO tokenDTO) {
        if (tokenDTO != null && tokenDTO.getToken() != null) {
            redisInvalidatedTokenService.invalidateToken(tokenDTO.getToken());
            return ResponseEntity.ok("Logged out successfully. JWT invalidated in Redis.");
        }
        return ResponseEntity.badRequest().body("Invalid token provided for logout.");
    }
    ...
    @SecurityRequirement(name = "onboarding-service")
